...
Elm.Elmsweep.make = function (_elm) {
   ...
   if (_elm.Elmsweep.values)
   return _elm.Elmsweep.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   c) {
      return function () {
         var _v8 = {ctor: "_Tuple5"
                   ...
                   ,_4: c.adj};
         switch (_v8.ctor)
         {case "_Tuple5": switch (_v8._0)
              {case false: switch (_v8._1)
                   {case false: switch (_v8._2)
                        {case false: switch (_v8._3)
                           ...
                           case true: return "F";}
                        break;
                      case true: switch (_v8._2)
                        {case false: return "";
                           case true: switch (_v8._3)
                             {case false: return "F";
                                case true: return "W";}
                             break;}
                        break;}
                   break;
                 case true: switch (_v8._1)
                   {case false: return "X";
                      case true:
                      return $Basics.toString(_v8._4);}
                   break;}
              break;}
         return "U";
      }();
   });
   var statusString = F2(function (won,
   lost) {
      return won ? ":D" : lost ? ":(" : ":)";
   });
   var screen = function (str) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("esw-screen")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("esw-noselect")]),
      _L.fromArray([$Html.text(str)]))]));
   };
   var adjacent = F2(function (i,
   _v14) {
      return function () {
         switch (_v14.ctor)
         {case "Board":
   ...
   var makeBoard = F3(function (w,
   h,
   mines) {
      return function () {
         var three = F2(function (n,
         ls) {
            return A2($List.drop,
            n - 1,
            A2($List.take,n + 2,ls));
         });
         var adjMines = F3(function (x,
         y,
         bo) {
            return $List.length($Basics.fst(A2($List.partition,
            $Basics.identity,
            $List.concat(A2(three,
            y,
            A2($List.map,three(x),bo))))));
         });
         var rows = A2(split,w,mines);
         var makeCell = F3(function (x,
         y,
         mine) {
            return A4(Cell,
            false,
         ...
         w,
         h,
         $Array.fromList($List.concat(bo)));
      }();
   });
   var makeNewBoard = F2(function (w,
   h) {
      return A3(makeBoard,
      w,
      h,
      A2($List.repeat,w * h,true));
   });
   var Lost = {ctor: "Lost"};
   ...
   var Won = {ctor: "Won"};
   var gameWon = function (gs) {
      return _U.eq(gs.status,Won);
   };
   var Play = {ctor: "Play"};
   var makeNewGame = F4(function (w,
   h,
   n,
   seed) {
      return A7(GameState,
   ...
   var update = F2(function (cmd,
   gs) {
      return gameDone(gs) ? function () {
         switch (cmd.ctor)
         {case "InitGame":
            switch (cmd._0.ctor)
              {case "_Tuple4":
         ...
         {case "Click":
            switch (cmd._0.ctor)
              {case "Just":
                 switch (cmd._1.ctor)
                   {case "Show":
                      return function () {
                           var gs0 = _U.eq(gs.timeStart,
                           0) ? _U.replace([["timeReached"
                                            ,cmd._0._0]
                                           ,["timeStart",cmd._0._0]],
                           gs) : gs;
            ...
            return randomGame(gs);
            case "Timer":
            return _U.eq(gs.timeStart,
              0) ? gs : _U.replace([["timeReached"
                                    ,cmd._0]],
              gs);}
         return gs;
      }();
   ...
   var times = A2($Signal.map,
   Timer,
   $Time.every(1 * $Time.second));
   var commands = $Signal.channel(Restart);
   var statusScreen = function (str) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("esw-status")
                   ,$Html$Events.onClick(A2($Signal.send,
                   commands,
                   Restart))]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("esw-rotate-90")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("esw-noselect")]),
      _L.fromArray([$Html.text(str)]))]))]));
   };
   ...
   c) {
      return function () {
         ...
         var down = !_U.eq(str,"");
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class(down ? "esw-cell esw-cell-down" : "esw-cell esw-cell-up")
                      ,$Html$Events.onClick(A2($Signal.send,
                      commands,
                      show(i)))
                      ...
                      flag(i)))]),
         _L.fromArray([A2($Html.span,
         _L.fromArray([fieldClass(str)]),
         _L.fromArray([$Html.text(_U.eq(str,
         "0") || (_U.eq(str,
         "B") || (_U.eq(str,
         "W") || (_U.eq(str,
         "F") || _U.eq(str,
   ...
   var display = function (gs) {
      return function () {
         ...
         return _U.eq(gs.nmine,
         0) ? $Html.text("") : A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("esw-table")]),
         _L.fromArray([A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("esw-status")]),
                      _L.fromArray([screen(time)
                                   ,statusScreen(A2(statusString,
                                   won,
                                   lost))
                                   ,screen(flag)]))
                      ,A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("esw-rows")]),
                      A2($List.map,
                      function (row) {
   ...
   var initGame = _P.portIn("initGame",
